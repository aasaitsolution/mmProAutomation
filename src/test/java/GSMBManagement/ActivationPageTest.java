            ...
            try {
                ...
                for (int attempt = 0; attempt < 3; attempt++) {
                    try {
                                ...
                                for (WebElement row : rows) {
                                    ...
                                    if (!rowText.contains("no data") && !rowText.isEmpty()) {
                                        List<WebElement> buttons = row.findElements(By.xpath(
                                                ".//*[contains(translate(text(),'ACTIVATE','activate'),'activate') " +
                                                        "or contains(translate(text(),'DEACTIVATE','deactivate'),'deactivate')]"));
                        ...
                        System.out.println("Stale element in tab: " + tabLabel + " — retrying...");
                        Thread.sleep(1000);
                    }
                }
                ...
                System.out.println("❌ Failed in tab: " + tabLabel);
                e.printStackTrace();
            }
        }
    }
